...
# questions
# 1. Does the diets affect life history traits? yes
# 2. do the microorganisms affect the response to life history traits? yes, plus they can be axenic on most diets
# 3. what is the relationship between the traits when colonized by different microbes on different diets? 
# 4. how is microbiota composition dteermiend by these? 
...
# starve2b <- starve2 %>% filter(vial %in% contaminated) # 3136
# 
# 
# starve2b <- starve2 %>%
#   filter()
...
# here's code to filter against, etc.... 
# starve2 <- starve2 %>%
#   mutate(fe = paste0(toupper(fruit), gsub(pattern = "exp",replacement = "",x = experiment))) %>%
#   filter(fe %in% toupper(c("asa","caa","fja","lia","maa","ora","spa","asb","bab","cab","mab","pob","rpb","cac","fjc","poc","cod","lid","tod", "prb","spb"))) %>%
# #  filter(fruit %in% c("BA","LI","MA","RP","SP"))
